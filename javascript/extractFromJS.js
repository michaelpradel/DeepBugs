...
(function () {
  ...
  const usage = `
Two usage modes:
  1) extractFromJS.js <what> <type> --parallel N <fileList.txt> <dir>
  ...
  2) extractFromJS.js <what> <type> --files <list of files>:
...
`;
  ...
  function spawnInstances (nbInstances, jsFiles, what, type) {
    const worklist = [];
  ...
  if (args[2] === '--parallel') {
    ...
    spawnInstances(nbInstances, jsFiles, what, type);
    ...
    const fileName = what + '_' + type + '_' + Date.now() + '.json';
